...
class Numar {
 ...
 getIban(person, cd){
  ...
  return cb(null, this.ibans[person]);
 }
};
...
.matches('identitate', (session, args) =>{
 let person = "";
 return model.getIban(person, (err, result) =>{
  if (err) {
   return session.send("Nu exista persoana");
  }
  return session.send('${person} are IBAN-ul ${result}');
 });
})
